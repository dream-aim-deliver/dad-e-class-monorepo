...
describe('Carousel', () => {
    ...
    it('renders nothing when children array is empty', () => {
        const { container } = render(
            <Carousel children={[]} locale="en" className="test-carousel" />,
        );
        ...
        if (carousel) {
            ...
            await vi.waitFor(
                () => {
                    expect(screen.getByText('Card 4')).toBeInTheDocument();
                    expect(
                        screen.queryByText('Card 1'),
                    ).not.toBeInTheDocument();
                    expect(
                        screen.queryByText('Card 2'),
                    ).not.toBeInTheDocument();
                    expect(
                        screen.queryByText('Card 3'),
                    ).not.toBeInTheDocument();
                },
                { timeout: 500 },
            );
