...
vi.mock('@maany_shr/e-class-translations', () => ({
  ...
  describe('GeneralCard', () => {
    ...
    it('shows tooltip container when title is truncated', async () => {
      ...
      // Find the title within the h2 element specifically
      const titleElement = screen.getByRole('heading', { name: 'Card Title' });
      const tooltipContainer = titleElement.parentElement?.querySelector(
        'div.bg-card-stroke.text-text-primary.text-sm'
      );
      expect(tooltipContainer).toBeInTheDocument();
      expect(tooltipContainer).toHaveTextContent('Card Title');
