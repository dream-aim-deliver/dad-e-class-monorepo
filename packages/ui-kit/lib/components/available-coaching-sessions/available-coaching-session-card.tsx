...
export interface AvailableCoachingSessionCardProps {
    title?: string;
    time?: number;
 ...
 */
export const AvailableCoachingSessionCard: FC<
    AvailableCoachingSessionCardProps
> = ({
    title,
    time,
    numberOfSessions,
    ...
    return (
        <>
            {isLoading ? (
                ...
                <div
                    className={cn("flex gap-2 p-2 items-center justify-between bg-card-stroke border border-divider rounded-medium w-full", onClick ? "cursor-pointer hover:bg-card-fill" : "")}
                    onClick={onClick}
                >
