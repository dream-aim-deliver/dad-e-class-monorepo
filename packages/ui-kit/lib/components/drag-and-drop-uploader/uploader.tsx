...
export const Uploader: React.FC<UploaderProps> = (props) => {
    ...
    const getUploaderText = () => {
        switch (variant) {
            case 'image':
                return {
                    title: dictionary.components.uploadingSection.uploadImage
                        .choseImages,
                    description:
                        dictionary.components.uploadingSection.uploadImage
                            .description,
                    maxSizeText:
                        dictionary.components.uploadingSection.maxSizeText,
                };
            case 'video':
                return {
                    title: dictionary.components.uploadingSection.uploadVideo
                        .choseVideos,
                    description:
                        dictionary.components.uploadingSection.uploadVideo
                            .description,
                    maxSizeText:
                        dictionary.components.uploadingSection.maxSizeText,
                };
            ...
            default:
                return {
                    title: dictionary.components.uploadingSection.uploadFile
                        .choseFiles,
                    description:
                        dictionary.components.uploadingSection.uploadFile
                            .description,
                    maxSizeText:
                        dictionary.components.uploadingSection.maxSizeText,
                };
    ...
    const handleUpload = async (selectedFiles: File[]) => {
        ...
        const newUploads: fileMetadata.TFileMetadata[] = filesToProcess.map(
            (file) => {
        ...
        const uploadPromises = newUploads.map(async (processingFile) => {
            const originalFile = filesToProcess.find(
                (f) =>
                    f.name === processingFile.name &&
                    f.size === processingFile.size,
            );
            if (!originalFile) return;

            const controller = abortControllers.current.get(
                processingFile.id as string,
            );
            ...
            try {
                ...
                setUploadingFiles((prev) =>
                    prev.filter((f) => f.id !== processingFile.id),
                );
            }
        });
