...
export const AnimatedRadioButton: FC<AnimatedRadioButtonProps> = ({
    ...
    const { container, indicator, translate } = sizeConfig[size];

    const containerClasses = cn(
        container,
        'relative inline-block rounded-full overflow-hidden cursor-pointer transition-colors duration-200',
        {
    ...
    const indicatorClasses = cn(
        indicator,
        'absolute top-[2px] left-[2px] rounded-full bg-white shadow-md transition-transform duration-300 ease-in-out',
        {
            [translate]: checked,
