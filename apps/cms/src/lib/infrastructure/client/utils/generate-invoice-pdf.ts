...
export async function generateInvoicePdf(options: GenerateInvoicePdfOptions): Promise<void> {
  ...
  // Only add footer text if company information is available
  const companyInfo = [platformData.companyName, platformData.companyAddress, platformData.companyUid]
    .filter(Boolean)
    .join(', ');

  if (companyInfo) {
    const footerText = document.createElement('div');
    footerText.textContent = companyInfo;
