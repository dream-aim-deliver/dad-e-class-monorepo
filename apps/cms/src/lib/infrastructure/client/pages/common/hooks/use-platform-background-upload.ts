...
export interface PlatformBackgroundUploadState {
...
/**
 * Custom hook for platform background image uploads
 * Uses requestFileUpload with uploadType: "upload_platform_background_image"
 * @param onProgressUpdate - Optional callback to track upload progress (0-100)
 */
export const usePlatformBackgroundUpload = (
    onProgressUpdate?: (progress: number) => void,
): PlatformBackgroundUploadState => {
    ...
    const uploadFile = async (
        ...
        // Request upload credentials from server for platform background image
        const uploadResult = await requestFileUploadMutation.mutateAsync({
            upload: {
                ...
                uploadType: "upload_platform_background_image",
            }
        });
    ...
    const handleFileUpload = async (
        ...
        try {
            ...
            if (error instanceof AbortError) {
                console.warn('Platform background image upload cancelled by user');
            } else {
                console.error('Platform background image upload failed:', error);
    ...
    const handleFileDelete = (id: string) => {
        ...
        console.log('Platform background image marked for deletion:', id);
    };

    const handleFileDownload = async (id: string) => {
        // Download functionality would require fetching the file metadata first
        console.log('Download platform background image:', id);
    };
