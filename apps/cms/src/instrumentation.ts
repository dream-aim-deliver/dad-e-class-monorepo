 ...
 *
 * Resource attributes:
 * - app.type: 'cms' - identifies this as the CMS app
 * - app.layer: 'server' - identifies server-side spans
 *
 * Note: page.route is captured at the browser level via resource attributes.
 * Server-side spans already contain the route in the span name (e.g., "GET /en/admin/courses").
 *
...
export async function register() {
    ...
    registerOTel({
        serviceName: process.env.OTEL_SERVICE_NAME || 'e-class-cms',
        attributes: {
            'app.type': 'cms',
            'app.layer': 'server',
        },
