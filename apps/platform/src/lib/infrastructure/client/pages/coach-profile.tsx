...
import { DefaultLoading, DefaultError, DefaultNotFound, Button, IconChevronLeft, Breadcrumbs, BookSessionWith, BuyCoachingSessionBanner, CourseCardList, CourseCard, Dropdown, IconFilter, CoachReviewCard, CoachReviewFilterModel, CoachReviewFilterModal, ReviewModal, ReviewDisplay, CoachReviewCardList } from '@maany_shr/e-class-ui-kit';
...
export default function CoachProfile({ username }: CoachProfileProps) {
	...
	const [coachCoursesResponse, { refetch: refetchCoachCourses }] = trpc.listCoachCourses.useSuspenseQuery({
		...
		// TODO: In backend please except username instead coachId
		// coachUsername: username,
	});
	...
	// Helper function to render course cards for students
	const renderStudentCourseCard = (course: any) => {
					...
					<Breadcrumbs
						items={[
							...
							},
							{
								...
								onClick: () => route.push('/coaches'),
							},
							{
								label: `${coachIntroduction.name} ${coachIntroduction.surname}`,
								onClick: () => route.push(`/coaches/${username}`),
							},
						]}
					/>
				</div>
			...
			{isLoggedIn && (
				<div className='flex flex-col gap-4'>
					...
					<CourseCardList
						...
						emptyStateButtonText={coachProfileTranslations('exploreCourses')}
					>
						{coachCourses.map((course) => renderStudentCourseCard(course))}
					</CourseCardList>
				</div>
			)}
